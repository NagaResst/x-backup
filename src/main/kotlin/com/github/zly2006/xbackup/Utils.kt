...
//? if >=1.21.11 {
/*import net.minecraft.server.permissions.Permissions as McPermissions
*///?}
...
object Utils {
    ...
    fun MinecraftServer.broadcast(text: Component) {
        playerList.players
            .filter {
                //? if >=1.21.11 {
                /*it.createCommandSourceStack().permissions().hasPermission(McPermissions.COMMANDS_GAMEMASTER)
                *///?} else {
                it.createCommandSourceStack().hasPermission(2)
                //?}
            }
            .forEach { it.sendSystemMessage(text) }
    }
